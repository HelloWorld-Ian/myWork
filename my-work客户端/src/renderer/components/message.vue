  ...
  <el-dialog  title="计划信息" v-if="planDetailVisible"  :visible.sync="planDetailVisible">
    ...
    <el-descriptions-item label="任务时间">{{proSlected.startDate+" "+
     ...
     <!-- 任务进度转换弹窗 -->
      ...
      <el-card >
      <el-tag type="success">任务进度已更新</el-tag>
    ...
    methods:{
       ...
       messageTitle(message){
        ...
        save(msg){
            ...
            if(that.$store.state.login){                   
                    that.$store.state.syncing=true
                    that.$http.post(that.$store.state.contextUrl+'/getPlanId',that.$qs.stringify({
                        user_id:that.messageSlected.body.sender,
                        plan_name:that.messageSlected.body.content.plan_name
                    })).then(function(res){
                        let plan_id=parseInt(res.data)
                        if(plan_id==-1){
                            that.$message({
                                message:'该计划不存在，无法添加到我的合作',
                                type:'error'
                            })
                            return 
